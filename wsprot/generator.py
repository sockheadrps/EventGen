"""
Code generator for Event Protocol definitions.
...
'''
    ...
    def _generate_js_client(self) -> str:
        """Generate a vanilla JavaScript client SDK with pluggable transport."""
        c2s = self.protocol.client_to_server_events()
        s2c = self.protocol.server_to_client_events()
        ...
        lines = [
            f"/**",
            f" * {class_name} - Auto-generated event client",
            f" * Protocol: {self.protocol.name} v{self.protocol.version}",
            f" * ",
            f" * Transport-agnostic: works with WebSocket, message queues, or any JSON transport.",
            f" */",
            "",
        ]
        ...
        for event in global_events:
            const_name = event.name.upper().replace(".", "_").replace("-", "_")
            lines.append(f"    {const_name}: '{event.name}',")
        
        # Feature groups
        sorted_features = sorted([f for f in features.keys() if f is not None])
        for feature in sorted_features:
            class_name_inner = feature.replace(" ", "").replace("-", "")
            lines.append(f"    {class_name_inner}: {{")
            for event in features[feature]:
                if "." in event.name:
                    partial_name = event.name.split(".", 1)[1]
                else:
        ...
        lines.extend([
            "/**",
            " * Create a transport adapter from a WebSocket.",
            " * @param {WebSocket} ws - WebSocket instance",
            " * @returns {Object} Transport adapter with send() and onMessage()",
            " */",
        ...
        lines.extend([
            ...
            f"     * @param {{Object}} transport - Transport with send(data) and onMessage(callback)",
            f"     */",
            f"    constructor(transport) {{",
            f"        this.transport = transport;",
            f"        this.listeners = {{}};",
            f"        ",
            f"        if (transport && transport.onMessage) {{",
            f"            transport.onMessage((data) => this._handleMessage(data));",
            f"        }}",
            f"    }}",
            "",
            ...
            "",
            f"    _send(data) {{",
            f"        if (this.transport && this.transport.send) {{",
            f"            this.transport.send(data);",
            f"        }}",
            f"    }}",
            ""
        ])
...
<head>
    <meta charset="UTF-8">
    <title>Event Protocol Client</title>
    <script src="{script_src}" type="module"></script>
</head>
<body>
    <h1>Event Protocol Client</h1>
